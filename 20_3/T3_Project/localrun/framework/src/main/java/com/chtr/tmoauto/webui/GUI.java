...
public class GUI 
{
	...
	public String fw_get_list_of_test_cases_to_execute(String input_filename) throws IOException, InterruptedException
	{
		...
		for (int x=2;x<Row+1;x++)
		{
			...
			if (ep_sheetname.equals("Environment"))
			{
				environment_value = ep_value;
				fw_set_variable("SKIP--ENVSELECTED", environment_value);
			}
	...
	public void fw_create_environment_variables(String environment_value, String configuration_map_fullpath) throws IOException, InterruptedException
	{
		...
		int ColEnvironment = xls.getColumnCount("Environment");
		
		String cur_value = "";
		String cur_variable = "";
		
		int row_value = 0;
		for (int x=0;x<RowEnvironment+1;x++)
		{
			cur_value = xls.getCellData("Environment", 0, x);
			if (cur_value.equals(environment_value))
			{
				row_value = x;
				break;
			}
		...
		else
		{
			...
			for (int y=0;y<ColEnvironment+1;y++)
			{
				cur_value = xls.getCellData("Environment", y, row_value);
				cur_variable = xls.getCellData("Environment", y, 1);
				if (!cur_variable.isEmpty())
				{
					//System.out.println("ENV" + cur_variable + "-" + cur_value);
					fw_set_variable("ENV" + cur_variable, cur_value);
				}
			}
			
		}
		
		log.fw_writeLogEntry("Create Environment Variables (Environment: " + environment_value + ", Configuration Map Full Path: " + configuration_map_fullpath + ")" + text_msg, ret_cd);		
		...
		while (continue_flag) 
		{
			...
			for (int i=0;i<tagname_list_len;i++)
			{
				...
				else
				{
					...
					{	
						for (int j=cur_pos_string;j<100000;j++)
						{
