...
public class T3Tests {
	...
	/**
	 * This function is main entry into the execution of all T3 test casess.
	 * 
	 * @param args
			...
			for (int y = 2; y < Row + 1; y++) {
				...
				if (tc_eventname.equals("Component")) {
					...
					for (int w = 2; w < RowComponent + 1; w++) {
						...
						tc_comp_name = "";
						if (tc_objectname_comp.contains("--") || (tc_comp_name.contains(tc_testdata))){							//check if there is -- in the step in component sheet
						...
						// End of Updated by Rakesh/Gaurav on 2/24/2017
							Long s = Long.parseLong(xls.getCellData("Component", "TCStep", w));
							Long s1 = Long.parseLong(fwgui.fw_get_value_from_file(workspace_name + "\\variables\\DebugLine"));
							if(s==s1)
							{
