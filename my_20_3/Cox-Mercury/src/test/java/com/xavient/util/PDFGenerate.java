	...
	public static void createPDF(String des) throws DocumentException, MalformedURLException, IOException
	{
		...
		if(skipCount>=1)
		{
			for(int i=1;i<=skipCount;i++)
			{
				...
				tableS.addCell(f3);
				PdfPCell f6=null;
				if(CustomReports.testSkiped.get("skipError"+i)!=null)
				{
					f6 = new PdfPCell(new Phrase(CustomReports.testSkiped.get("skipError"+i).toString()));
				} else{
					f6 = new PdfPCell(new Phrase("UNKNOW ERROR APPEARED"));
				}
				f6.setColspan(1);
				f6.setBackgroundColor(BaseColor.LIGHT_GRAY);
				tableS.addCell(f6);	
