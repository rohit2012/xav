...
public class PDFListener implements ITestListener {
	...
	public void onTestSuccess(ITestResult result) {
		...
		p = new Paragraph("PASSED TEST -" + result.getMethod().getDescription(), new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.BOLD));
		p.setAlignment(Element.ALIGN_CENTER);
		cell = new PdfPCell(p);
		cell.setColspan(4);
		cell.setBackgroundColor(Color.LIGHT_GRAY);
	...
	{
		log("onStart("+context+")");
		file = new File("D:\\Folder Strucure\\Projects\\Charter Communication\\workspace\\TestJava\\File\\Test4.pdf");
...
/*	@BeforeClass
	public void test(ITestContext c)
	{
		PDFListener();
		onStart(c);
	}
	...
	@AfterMethod
	public void a(ITestResult r)
	{
		
		
		if(r.SUCCESS==1)
		{
			System.out.println("Pass : "+r.getName());
			onTestSuccess(r);
		} else if(r.FAILURE==2)
		{
			System.out.println(r.getStatus()+" "+r.FAILURE);
			onTestFailure(r);
		}
	...
	@AfterClass
	public void afterClass(ITestContext c)
	{
		onFinish(c);
	} */
}
